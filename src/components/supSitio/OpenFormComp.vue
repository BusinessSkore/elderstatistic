<template>
  <!-- {{ this.filteredPacientesAEgresarData }} -->
        ...
        <form>
          <fieldset>
            <!-- <label
              v-if="
                this.evaluacion.status !== '3' &&
                  this.evaluacion.cantidadEvaluaciones >= this.altaLimit
              "
              style="color: white; background-color: orange"
              class="form-label"
              ><b
                >Este Paciente está activo y tiene 17 evaluaciones o más, por lo
                que debe ser dado de alta</b
              ></label
            > -->
            <label
              v-if="this.estaMatrizEgresoData"
              style="color: white; background-color: orange"
              class="form-label"
              ><b
                >Este paciente se encuentra en la matriz de egresados</b
              ></label
            >
            <h6>{{ encabezado }}</h6>
            ...
            <button
              ...
              @click="
                this.$router.push(`/listevalssupSitio2/${evaluacion._id}`)
              "
            >
...
import pacientesAEgresarFile from "./pacientesAEgresar.json";
...
export default defineComponent({
  ...
  data() {
    return {
      pacientesAEgresarData: pacientesAEgresarFile,
      filteredPacientesAEgresarData: [],
      estaMatrizEgresoData: false,
      altaLimit: 17,
  ...
  methods: {
    estaMatrizEgreso(cedula: string) {
      // alert(`Verificando cedula No.: ${cedula}`);
      this.filteredPacientesAEgresarData = this.pacientesAEgresarData.filter(
        (paciente: any) => {
          return paciente.cedula.includes(cedula);
        }
      );

      if (this.filteredPacientesAEgresarData.length == 1) {
        this.estaMatrizEgresoData = true;
      } else {
        this.estaMatrizEgresoData = false;
      }
    },
    ...
    setPatienData(
      cantEvals: number,
      patient: any,
      patientFechaNacimiento: any
    ) {
      // alert(cantEvals)
      // alert(patient)
      // alert(patientFechaNacimiento)
      if (patient.status == "3") {
        alert(
          `Paciente Egresado, No puede ser evaluado. Fecha: ${patient.fecha}, UNAP: ${patient.unap}, Médico: ${patient.nombre_doctor}`
        );
        this.evaluacion.cedula = "";
      ...
      this.calcularTiempoUltEval(patient.fecha);
      ...
      if (patient) {
        ...
        this.evaluacion.direccion = patient.direccion;
    ...
    async defCedula() {
      ...
      if (this.evaluacion.cedula.length == 11) {
        ...
        this.estaMatrizEgreso(this.evaluacion.cedula);

        // Verificar si tiene la evaluacion ese día
        this.toggleSpinner();
        try {
          ...
          // Buscando Fecha de Nacimiento Vieja
          const patientFechaNacimientoData = await getPatientFechaNacimiento(
            this.evaluacion.cedula
          );

          // Utilizando los Datos Unidos en el Paciente (Con control de No Encontrado Diferente a Cédula == 13)
          if (
            patientData.data.title == "Usuario no Encontrado" ||
            patientFechaNacimientoData.data.title == "Usuario no Encontrado"
          ) {
            // alert("No Encontrado");
            // alert(JSON.stringify(patientData.data.Items.length))
            this.setPatienData(
              patientData.data.Items.length,
              patientData.data.Items[patientData.data.Items.length - 1],
              patientData.data.Items[patientData.data.Items.length - 1]
            );
          } else {
      ...
      // Cédula con guiones guiones
        ...
        try {
          ...
          // Uniendo Evaluaciones Viejas y Nuevas
          for (var i = 0; i < patientData2.data.Items.length; i++) {
            patientData.data.Items.push(patientData2.data.Items[i]);
          }
          ...
          // Utilizando los Datos Unidos en el Paciente
      ...
      // Cédula Con longitudes incorrectas, ni 11 ni 13
      else {
        alert("Por Favor, Verifique la Cédula");
        document.getElementById("cedula").focus();
      }
    },
    setContactName(nombre_contacto: string) {
      if (this.evaluacion.tipo_de_contacto == "PROPIO") {
        this.evaluacion.nombre_contacto =
          this.evaluacion.nombre + " " + this.evaluacion.apellido;
      } else {
        if (nombre_contacto) {
          this.evaluacion.nombre_contacto = nombre_contacto;
        } else {
          this.evaluacion.nombre_contacto = "";
        }
      }
    ...
    calcularTiempoUltEval(date: any) {
      let days = 0;
      let tiempoTrans = Math.floor(
        moment(new Date()).diff(moment(date, "YYYY-MM-DD"), "days", true)
      );
      // if (edad > 124 || edad < 0) {
      //   console.log("Verificar Fecha de Nacimiento");
      // } else {
      ...
      if (tiempoTrans <= 31) {
        if (
          confirm(
            `Este Paciente fue evaluado por última vez el dia ${date} (hace ${tiempoTrans} dias), ¿Desea continuar?`
          )
        ) {
          alert(
            "El Paciente será evaluado, aunque no cumple con el criterio de los 31 dias"
          );
        } else {
          alert(
            "El Paciente no será evaluado, en cumplimiento del criterio de los 31 dias"
          );
          this.evaluacion.cedula = "";
          this.evaluacion.nombre = "";
          this.evaluacion.apellido = "";
    ...
    fillFields() {
      ...
      if (
        ...
        this.$store.state.user.type == "Médico Nutricionista Domiciliario"
      ) {
