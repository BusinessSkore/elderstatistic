<template>
  ...
  <table id="customers">
    ...
    <tr v-for="(evaluacion, index) in evaluacions" :key="index">
      ...
      <td>
        <i
          @click.prevent="this.loadHistorial(evaluacion.cedula)"
          style="cursor: pointer"
          class="fas fa-list"
        ></i>
      </td>
...
export default {
  ...
  methods: {
    ...
    async loadHistorial(cedula: string) {
      // Consulta Evaluaciones Viejas
      const patientData = await getPatient(cedula);
      ...
      const patientData2 = await getManyByCedula(cedula);
